...
CustomWindowSuccessful::CustomWindowSuccessful(const QString& message, QWidget* parent) : QWidget(parent) {
    ...
    QTimer::singleShot(2000, this, &CustomWindowSuccessful::close);
}


void CustomWindowSuccessful::updatePosition(QWidget* parent) {
    if (parent) {
...
void CustomWindowSuccessful::showEvent(QShowEvent* event) {
    ...
    animation->setEndValue(1);   // End transparency
    animation->start();
}