...
void QTPainter::saveToImage(const QString& fileName, QString& format) {
    ...
    if (chosenFormat == "SVG") {
        ...
        generator.setSize(QSize(rectangle->width(), rectangle->height()));
        generator.setViewBox(QRect(0, 0, rectangle->width(), rectangle->height()));
        generator.setTitle("Exported SVG");
        generator.setDescription("Generated by QTPainter");
        QPainter painter(&generator);
        drawGostFrame(&painter, QSize(rectangle->width(), rectangle->height()));
        painter.translate((qint32) (rectangle->width()/ 2), (qint32) (rectangle->height() / 2));
        ...
        QPixmap pixmapCopy(QSize(rectangle->width() + 40, rectangle->height() + 40));
        pixmapCopy.fill(Qt::white);
        QPainter painter(&pixmapCopy);
        drawGostFrame(&painter, QSize(rectangle->width() + 40, rectangle->height() + 40));

        painter.translate((qint32) (rectangle->width()/ 2) + 20, (qint32) (rectangle->height() / 2 + 20));
