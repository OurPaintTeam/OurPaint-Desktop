...
#include "../viewport/IViewportController.h"
#include "Logger.h"
...
int QtViewportHost::setEventSink(IViewportController* controller) {
    if (!controller) {
        controller_ = nullptr;
        context_ = nullptr;
        return -1;
...
void QtViewportHost::setActiveController(int index) {
    ...
    const int wLogical = width();
    const int hLogical = height();

    const int wFb = int(wLogical * devicePixelRatio());
    const int hFb = int(hLogical * devicePixelRatio());

    glViewport(0, 0, wFb, hFb);

    input::ResizeEvent event;
    event.width = wLogical;
    event.height = hLogical;
